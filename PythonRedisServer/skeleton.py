...
# Exceptions for connection handling and command errors
...
class ProtocolHandler(object):
    ...
    def handle_request(self, socket_file):
        ...
        # Convert bytes to string for dictionary lookup in Python 3
    ...
    def handle_string(self, socket_file):
        ...
        line = socket_file.readline().rstrip(b'\r\n')
        if not line:
            raise Disconnect()
        length = int(line)
        if length == -1:
...
class Server(object):
    ...
    def mset(self, *items):
        data_list = list(zip(items[::2], items[1::2]))
...
if __name__ == '__main__':
    ...
    print('Redis server running on 127.0.0.1:31337')
    Server().run()
